// ============================================================================
// Nexus Agent Platform — ExecutionLoop v2 (Registry-Integrated)
// ============================================================================
...
//
// KEY CHANGE vs v1: All agent adapters are resolved through the
// AgentRegistry instead of being baked into GraphNode instances.
// ============================================================================
...
// ============================================================================
// ExecutionLoop (v2 — Registry-Integrated)
// ============================================================================
...
export class ExecutionLoop {
  ...
  constructor(
    ...
    // NOTE: Runners are NOT pre-created here in v2.
    // They are created lazily via this.registry.get() on first execution.
    // This ensures adapters are always resolved through the registry.
  ...
  async execute(context: AgentContext): Promise<GraphResult> {
    ...
    const routerSkipped = new Set<string>();
    ...
    for (const level of levels) {
      ...
      // After a level completes, propagate conditional router skips
  ...
  // ========================================================================
  // Standard Node Execution (Registry-Integrated)
  // ========================================================================

  /**
   * Execute a standard agent node using AgentRunner with retry logic.
   * The adapter is resolved through the AgentRegistry rather than
   * being used directly from the GraphNode.
   */
  private async executeStandardNode(
    ...
    try {
      ...
      // ── Resolve adapter through the registry ─────────────────────────
      const agentId: string = node.agent?.metadata?.id ?? nodeId;
      const adapter: IAgentAdapter = this.registry.get(agentId);

      // ── Build or retrieve runner ──────────────────────────────────────
      let runner = this.runners.get(nodeId);
      if (!runner) {
        runner = new AgentRunner(
          // The registry returns IAgentAdapter which is structurally
          // compatible with AgentRunner's expected adapter type at
          // runtime (actual adapters implement EnhancedAgentAdapter
          // which includes validators, inputSchema, outputSchema, etc.).
          adapter as any,
          {
      ...
      const result: AgentResult = await runner.run(input, context);

      // Map output on success
      if (result.status === "completed" && result.data !== null) {
        const output = {
          schema: runner.adapter.outputSchema.$id ?? agentId,
      ...
      return this.buildNodeResult(nodeId, agentId as AgentId, agentStatus, result, {
        startedAt,
        error: result.error,
      });
    } catch (error) {
      ...
      return this.buildNodeResult(
        nodeId,
        (node.agent?.metadata?.id ?? nodeId) as AgentId,
        "failed",
        null,
        { startedAt, error: errorMsg },
      );
    }
  }
